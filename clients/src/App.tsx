        ...
        >
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="student1 or admin1"
                required
              />
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 glass px-3 py-2 rounded-lg text-xs text-gray-400 hover:text-white hover:bg-white/10 transition-all"
            >
              👨‍🎓 student1
            </button>
            <button
...
function StudentDashboard({
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      // Safety net: Force it to be an array even if the backend sent a string
      const parsedData = typeof data === "string" ? JSON.parse(data) : data;
      setOrders(Array.isArray(parsedData) ? parsedData : []);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    fetchMenu();
    fetchOrders(); // <-- Add this to load history immediately
    return () => wsRef.current?.close();
  }, [connectWebSocket, fetchMenu, fetchOrders, token]);
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative hidden md:block">
              ...
              <input
                ...
                className="input-field pl-9 pr-4 py-2.5 rounded-xl w-64 text-sm"
              />
            </div>
            <motion.button
      ...
      <div className="md:hidden px-6 pt-4">
        <div className="relative">
          ...
          <input
            ...
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search menu..."
            className="input-field pl-9 pr-4 py-2.5 rounded-xl w-full text-sm"
          />
        </div>
      </div>
                  ...
                  <div className="p-5 pt-2">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <motion.button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          item.availableQty > 0 && onAddToCart(item);
                        }}
                      >
                        + Add
                      </motion.button>
        ...
        >
          <div className="glass-card p-5 sticky top-24">
            <h3 className="text-xl font-extrabold mb-4 flex items-center gap-2">
              ...
              {cart.length > 0 && (
                <span className="ml-auto bg-purple-500/30 text-purple-300 text-xs font-bold px-2.5 py-1 rounded-full">
                  {cart.length}
                </span>
              )}
            </h3>
            {cart.length === 0 ? (
              ...
              <>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className="w-6 h-6 rounded-md bg-white/10 text-xs flex items-center justify-center hover:bg-white/20 transition"
                        >
                          −
                        </button>
                      ...
                      <button
                        ...
                        className="text-red-400 hover:text-red-300 text-sm ml-1 transition"
                      >
                        ✕
                      </button>
                    </motion.div>
                ...
                <div className="border-t border-white/10 pt-4 space-y-3">
                  ...
                  {error && (
                    <p className="text-red-400 text-xs bg-red-500/10 rounded-lg p-2">
                      {error}
                    </p>
                  )}
                  <motion.button
        ...
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <motion.button
              whileHover={{ x: -3 }}
              onClick={onGoMenu}
              className="text-gray-400 hover:text-white transition flex items-center gap-1 font-medium"
            >
          ...
          <div className="flex gap-3">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={onRefresh}
              className="glass px-4 py-2 rounded-xl text-sm hover:bg-white/10 transition font-medium"
            >
              🔄 Refresh
            </motion.button>
            <motion.button
          ...
          >
            {orders.map((order: Order, idx: number) => (
              <motion.div
                key={order.orderId}
                variants={fadeUp}
                className="glass-card p-5 hover:border-purple-500/20 transition-colors"
              >
                    ...
                    return (
                      <div key={step} className="flex items-center flex-1">
                        ...
                        {i < statusSteps.length - 1 && (
                          <div
                            className={`h-0.5 flex-1 mx-1 rounded transition-all ${isActive ? "bg-gradient-to-r from-cyan-400 to-purple-500" : "bg-gray-700"}`}
                          />
                        )}
                      </div>
                    );
...
function AdminDashboard({
  ...
  const pollServices = useCallback(async () => {
    ...
    const gw = updated.find((s) => s.key === "order-gateway"),
      ...
      kt = updated.find((s) => s.key === "kitchen-service"),
      nh = updated.find((s) => s.key === "notification-hub");
    setLatencyHistory((p) =>
      [
        ...p,
        {
          ...
          kitchen: kt?.metrics?.avgLatencyMs || 0,
          notification: nh?.metrics?.avgLatencyMs || 0,
        },
      ].slice(-30),
    );
    setOrdersHistory((p) =>
  ...
  const killService = async (svc: ServiceState) => {
    try {
      ...
      setChaosLog((p) => [`⚠️ Could not reach ${svc.name}`, ...p].slice(0, 20));
    }
  };
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
          ...
          >
            ...
            <span className="text-gray-400 flex items-center gap-2">
              👋{" "}
              <span className="font-semibold text-gray-300">System Admin</span>
            </span>
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-5 gap-4">
            {services.map((svc) => {
              ...
              return (
                ...
                >
                  <div
                    className={`absolute inset-0 transition-all duration-500 ${isDown ? "bg-red-500 opacity-20 animate-pulse" : justRecovered ? "bg-green-500 opacity-10" : svc.isUp ? "bg-green-500 opacity-[0.03]" : "bg-red-500 opacity-10"}`}
                  />
                  <AnimatePresence>
                    {isDown && (
                      <motion.div
                  ...
                  {justRecovered && svc.isUp && (
                    ...
                    >
                      <span className="text-[10px] bg-green-500/30 text-green-300 px-2 py-0.5 rounded-full font-bold">
                        ✅ Recovered
                      </span>
                    </motion.div>
                  )}
                  ...
                  <p
                    className={`text-xs font-bold uppercase tracking-wider ${isDown ? "text-red-400" : svc.isUp ? "text-green-400" : "text-red-400"}`}
                  >
                    ...
                    <div className="mt-3 flex flex-wrap gap-1 justify-center">
                      {Object.entries(svc.health.dependencies).map(
                        ([dep, info]) => (
                          <span
                            key={dep}
                            className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${info.status === "ok" ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
                          >
                            {dep}
                          </span>
                        ),
        ...
        <motion.div
          {...fadeUp}
          className="glass-card p-6 border-2 border-red-500/15 relative overflow-hidden"
        >
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-1">
              ...
              <span className="bg-red-500/20 text-red-400 text-[10px] font-extrabold uppercase tracking-widest px-3 py-1 rounded-full border border-red-500/30">
                Danger Zone
              </span>
            </div>
            ...
            <div className="grid grid-cols-5 gap-3 mb-5">
              {services.map((svc) => {
                ...
                return (
                  <motion.button
                    key={svc.key}
                    whileHover={svc.isUp && !isKilled ? { scale: 1.03 } : {}}
                    whileTap={svc.isUp && !isKilled ? { scale: 0.97 } : {}}
                    ...
                    className={`px-4 py-3.5 rounded-xl text-sm font-bold transition-all relative overflow-hidden ${
                      isKilled
                        ? "bg-red-900/30 border-2 border-red-500/40 text-red-300 cursor-not-allowed"
                        : svc.isUp
                          ? "bg-red-500/10 border-2 border-red-500/25 text-red-400 hover:bg-red-500/20 hover:shadow-lg hover:shadow-red-500/20 hover:border-red-500/50"
            ...
            {chaosLog.length > 0 && (
              <div className="bg-black/40 rounded-xl p-4 max-h-48 overflow-y-auto border border-white/5">
                ...
                {chaosLog.map((log, i) => (
                  ...
                  >
                    <div
                      className={`w-2 h-2 rounded-full shrink-0 ${log.includes("✅") ? "bg-green-400" : log.includes("💀") ? "bg-red-500" : "bg-yellow-500"}`}
                    />
                    <p
              ...
              <tbody>
                {services.map((svc) => (
                  ...
                  >
                    ...
                    <td className="py-3.5">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-bold ${svc.isUp ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
                      >
                        {svc.isUp ? "UP" : "DOWN"}
                      </span>
                    </td>
